# ADR-0005: テナントへのメール配信の要否と手段

- ステータス: Proposed（現時点では「作らない」。要望が出た時点で Workspace リレーで実装）
- 日付: 2026-08-14（2026-08-15 改訂: 当社 DNS の実態を確認し手段の推奨を反転、さらに「送らない」を第一案に）
...
**そもそも当社がメールを送らない**という選択肢を第一とする。理由は次の 3 点。

1. **データ配信は既に完了している。** レポートは**顧客自身のプロジェクトの GCS バケット**に置かれる（[ADR-0003](0003-signed-url-delivery.md)）。不足しているのは「届いたことの通知」だけであり、成果物そのものは渡っている。
1. **通知はテナント側で組む方が簡単。** 自社バケットの GCS 通知を自組織のメール／Slack に流すのは、**テナント側の管理者権限だけで完結**する。当社の情シス交渉も DNS 変更も不要になる。署名付きURLも本来は「当社側の人間に常設 IAM を配らない」ための仕組みであり、テナントが自社バケットを見るのに署名付きURLは要らない。
1. **要望が実在しない。** 現時点の実テナントは検証用 1 件のみ。メール配信を今作ると、情シス依頼・中継コンポーネント・バウンス監視という 3 つの継続的負債を、需要の確認前に抱えることになる。

したがって提供は次の 3 層とし、**上 2 層（実装済み）で運用を開始する**。

| 層           | 手段                                              | 当社の追加作業                           |
| :----------- | :------------------------------------------------ | :--------------------------------------- |
| 標準         | 顧客プロジェクトの GCS バケットへ配信（実装済み） | なし                                     |
| 希望者       | Slack Incoming Webhook の opt-in（実装済み）      | なし                                     |
| 要望が出たら | メール配信（下記の手段選定に従う）                | 情シス依頼＋中継 Cloud Run＋バウンス監視 |

## メール配信を行う場合の手段: Workspace SMTP リレー

実テナントからメール配信の要望が出た時点で着手する。その際は **Google Workspace SMTP リレーを採る。** 当初は SendGrid を推していたが、上記の実態を踏まえて反転させた。判断が変わった理由は「情シスへの依頼を避けられるか」ではなく（どちらも避けられない）、**依頼の大きさ**である。
...
- **Cloud Monitoring の通知チャネルで顧客へ送る**: 技術的には成立する（成功時に `report_url=…` を含むログを出し、`label_extractors` で抽出して `documentation` に埋め込めば、失敗通知と同じ仕組みで送れる）。しかし次の理由で採らない。

  - **レート制限で通知が消える。** ログベース条件では `notification_rate_limit` が必須で、当ポリシーは 300 秒。**同一ポリシーでは 5 分間に 1 通しか飛ばない**ため、週次実行で複数テナントの完了が重なると大半に届かず、しかも届かなかったことに気付けない。テナントごとにポリシーとチャネルを分ければ回避できるが、テナント数ぶんのアラートポリシーを運用することになる。
  - **件名に `[ALERT - …]` が自動で付く。** 成功レポートの案内が顧客に「アラート」として届く。接頭辞は消せない。
  - **当社の内部情報が載る。** 通知には `resource_container` や `workflow_id` 等のリソースラベルが含まれ、顧客に見せる内容ではない。
  - **用途の混在。** 監視の仕組みを成果物配信に転用すると、障害通知と顧客通知でチャネル・ポリシー設定が絡み合い、片方の変更が他方を壊す。[ADR-0001](0001-notification-channels.md) の「送信元の能力に手段を合わせる」に反し、能力の無い送信元へ用途を寄せる形になる。
